...
function Cdict_wv() {
	window.open("/common/advHjdict/popup.htm", "_blank", "status=yes,left=350,top=200,width=350,height=160");
}
function Cdict_wv_back(astring) {
...
function getSelection(){
	...
	if ((document.selection)&&(document.selection.type == "Text")) {
		...
		return range.text; 
	}else if (myArea.selectionStart!= undefined) {
		return  myArea.value.substr(myArea.selectionStart, myArea.selectionEnd - myArea.selectionStart);
	}else{
		return "";
	}
} 
function setSelection(astring){
	var obj= document.getElementById("Content"); 
	if(!isUndefined(obj.selectionStart)) { 
		var opn = obj.selectionStart + 0; 
		obj.value = obj.value.substr(0, obj.selectionStart) + astring + obj.value.substr(obj.selectionEnd);
		obj.focus();
	}else if ((document.selection)&&(document.selection.type == "Text")) {
		var range = document.selection.createRange();
		range.text=astring ; 
	}else{
		document.frmAnnounce.Content.value+=astring;
		document.frmAnnounce.Content.focus();
	}
} 
function Cdict_wjv() {
	window.open("/common/advHjdict_jp/popup.htm", "_blank", "status=yes,left=350,top=200,width=350,height=180");
}
function Cdict_wjv_back(astring) {
	setSelection(astring);
}
...
function fontchuli(){
	try{ 
		...
		if(!isUndefined(obj.selectionStart)) { 
			var opn = obj.selectionStart + 0;
			var ch_text =obj.value.substr(obj.selectionStart, obj.selectionEnd - obj.selectionStart);
			var txt = fontbegin + ch_text + fontend;
...
var currentTitle = document.frmAnnounce.subject.value; 
revisedTitle = currentTitle+addTitle; 
document.frmAnnounce.TopicFlagName.value = addTitle;
document.frmAnnounce.subject.value=revisedTitle; 
document.frmAnnounce.subject.focus(); 
return; }
...
/*新插入代码*/
function TextBoxEditor(atextArea){
	this.element=atextArea;
}
TextBoxEditor.prototype.getValue=function(){
	return this.element.value;
}		 
TextBoxEditor.prototype.getSelection=function(){
...
TextBoxEditor.prototype.replaceSelection=function(text){
...
TextBoxEditor.prototype.wrapSelection=function(before,after){
	this.replaceSelection(before+this.getSelection()+after);
}
TextBoxEditor.prototype.insertBeforeSelection=function(text){
	this.replaceSelection(text+this.getSelection());
}
TextBoxEditor.prototype.insertAfterSelection=function(text){
	this.replaceSelection(this.getSelection()+text);
}
TextBoxEditor.prototype.injectEachSelectedLine=function(callback,before,after){
	this.replaceSelection((before||'')+$A(this.getSelection().split("\n")).inject([],callback).join("\n")+(after||''));
}    

function showcolor2(color){
	aTextBoxEditor.wrapSelection('[color='+ color +']' , '[/color]')
}
function showsize2(size){
	aTextBoxEditor.wrapSelection('[size='+ size +']' , '[/size]')
}
